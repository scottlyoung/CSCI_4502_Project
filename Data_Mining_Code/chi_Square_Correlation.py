	...
	with open(sys.argv[4], "w") as output_file:
		...
		if (column1 != "None" and column2 != "None"):
			...
			for h in combo_dict:
				...
				final_dict[combo_dict[h][0]].append((combo_dict[h][1], correlation_score))
			...
			for j in final_dict:
				...
				output_file.write("===================\n")
				output_file.write("Average lift correlation for " + challenger + " and \"" + j +"\": "+str(summ/count)+"\n")
				output_file.write("===================\n")
				output_file.write("Highest Correlations:\n")
				for k in reversed(worker):
					output_file.write (k[0] + ": "+str(k[1])+"\n")
